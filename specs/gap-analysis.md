...
| 3 | Project config schema is missing uv/python/framework fields | `resolved` | `projectConfig` and local `.tahuna/tahuna.toml` parsing include `python_project_file`, `uv_lock_file`, `framework`, `version`, and `python_version` in `cli/project.go` |
