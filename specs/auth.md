...
Single-user email OTP authentication with CLI-initiated API key provisioning. No OAuth providers, no team/org accounts.
...
| Key expiry | 90 days from creation |
...
4. Reject if: not found, revoked (`revokedAt` set), or expired (> 90 days from creation).
...
- A user can have at most 5 active (non-revoked, non-expired) API keys.
...
| 5 keys already active | Oldest key is auto-revoked when new one is created. |
| Browser auth cancelled | CLI times out after 5 minutes, prints: "Login cancelled or timed out." |
