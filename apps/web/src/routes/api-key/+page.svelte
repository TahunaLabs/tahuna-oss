<script lang="ts">
  ...
  const defaultURL =
    (typeof import.meta !== "undefined" &&
      (import.meta as ImportMeta).env?.VITE_PUBLIC_TAHUNA_API_URL?.trim()) ||
    "http://localhost:8000";
  ...
  <form on:submit={onSubmit} class="space-y-5">
    ...
    {#if result}
      <Alert variant="success" className="mt-4 space-y-3">
        <p class="text-sm">
          Key ID: <span class="font-mono">{result.api_key_id}</span>
        </p>
        <div
          class="rounded-md border border-border bg-background p-3 font-mono text-xs break-all"
        >
          {result.api_key}
        </div>
        <pre
          class="rounded-md border border-border bg-background p-3 text-xs overflow-x-auto"><code
            >{exportSnippet}</code
          ></pre>
      </Alert>
    {/if}
